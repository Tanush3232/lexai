...
export default function NewIntelligencePage() {
  ...
  const submit = async () => {
    ...
    if (!sid) {
      try {
        const res = await chatApi.createSession({
          title: q.slice(0, 60),
          scope_type: "hybrid",
          scope_folder_ids: selectedFolderIds,
          scope_document_ids: selectedDocs.map(d => d.id),
        });
        sid = res.data.id;
        setSessionId(sid);
        // Update URL to session route without navigation
        window.history.replaceState({}, "", `/dashboard/intelligence/${sid}`);
      } catch {
    ...
    try {
      const res = await chatApi.sendMessage(sid!, q);
      ...
      <div style={{ display: "flex", flexDirection: "column", height: "calc(100vh - 48px)", maxWidth: "1100px", margin: "-20px auto -48px" }}>
        ...
        <div style={{ padding: "12px 32px 0", flexShrink: 0 }}>
          <button
            ...
            style={{ display: "flex", alignItems: "center", gap: 6, background: "none", border: "none", cursor: "pointer", color: "var(--text2)", fontSize: 13, fontWeight: 600, padding: "4px 0" }}
          >
        ...
        <div style={{ flex: 1, overflowY: messages.length === 0 ? "hidden" : "auto", padding: "20px 32px 16px" }}>
          {messages.length === 0 ? (
            <div style={{ height: "100%", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: "20px" }}>
              <div style={{ width: "76px", height: "76px", background: "var(--accent-light)", borderRadius: "22px", display: "flex", alignItems: "center", justifyContent: "center" }}>
                <MessageSquare size={38} style={{ color: "var(--accent)" }} />
              </div>
              <div style={{ textAlign: "center" }}>
                <h1 style={{ fontFamily: "var(--font-playfair)", fontSize: "32px", margin: "0 0 10px", color: "var(--text)" }}>Document Intelligence</h1>
                <p style={{ color: "var(--text2)", fontSize: "15px", maxWidth: "480px", lineHeight: "1.7", margin: 0 }}>
                  Ask anything about your legal documents. Select files from your vault for targeted analysis, or start chatting to search all documents.
                </p>
              </div>
            </div>
          ) : (
        ...
        <div style={{ padding: "0 20px 20px", flexShrink: 0 }}>
          ...
          <div style={{
            ...
            {sending ? (
              <button onClick={stop} title="Stop" style={{ width: "34px", height: "34px", borderRadius: "50%", border: "none", background: "var(--text)", color: "var(--white)", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", flexShrink: 0, padding: 0 }}>
                <Square size={13} fill="currentColor" />
              </button>
            ) : input.trim() ? (
              <button onClick={submit} title="Send" style={{ width: "34px", height: "34px", borderRadius: "50%", border: "none", background: "linear-gradient(135deg, var(--accent) 0%, #6366f1 100%)", color: "#fff", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", flexShrink: 0, boxShadow: "0 3px 12px rgba(79,70,229,0.35)", padding: 0 }}>
