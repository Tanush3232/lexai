...
function VaultModal({ folders, selectedDocs, selectedFolderIds, onToggleDoc, onToggleFolder, onClose }: VaultModalProps) {
  ...
  return (
    <div
      style={{ position: "fixed", inset: 0, zIndex: 1000, background: "rgba(0,0,0,0.45)", backdropFilter: "blur(4px)", display: "flex", alignItems: "center", justifyContent: "center" }}
      onClick={onClose}
    >
      <div
        style={{ width: "520px", maxHeight: "68vh", background: "var(--white)", borderRadius: "18px", boxShadow: "0 24px 80px rgba(0,0,0,0.18)", display: "flex", flexDirection: "column", overflow: "hidden" }}
    ...
    <div style={{ paddingLeft: "30px", paddingBottom: "4px", display: "flex", flexDirection: "column", gap: "2px" }}>
      {docs.map((d: any) => {
        ...
        return (
          <div
            key={d.id}
            onClick={() => onToggleDoc({ id: d.id, name: d.name, folder_name: folderName })}
            style={{ display: "flex", alignItems: "center", gap: "8px", padding: "7px 10px", borderRadius: "8px", background: sel ? "var(--accent-light)" : "transparent", cursor: "pointer", transition: "background 0.15s" }}
          >
